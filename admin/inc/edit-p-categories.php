...
/**************************** EDIT DANH MỤC SẢN PHẨM **********************************/
			...
			$dcid = NULL;
			//Lấy dữ liệu từ CSDL
			$q = "SELECT cat_name, position FROM p_categories WHERE cat_id = {$ecid}";
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {//Nếu đúng -> Form đã được submit -> xử lý form
				...
				if(empty($errors)) { //Nếu không có lỗi xảy ra thì chèn vào CSDL
					//Cập nhật
					$q = "UPDATE p_categories ";
					$q .= " SET cat_name = '{$p_cat_name}', position = $position ";
					$q .= " WHERE cat_id = {$ecid}";
					$r = mysqli_query($dbc, $q);
						confirm_query($r, $q);
					if (mysqli_affected_rows($dbc) == 1) {
						redirect_to('admin/p-categories.php?msg=2');
					} else {
				...
				<div>
					<form action="" method="POST" id="add-n-cat" class="add-form">
						<fieldset>
							...
							<input type="text" name="p-category" id="p-category" value="<?php if(isset($ecat_name)) echo $ecat_name; ?>" size="20" maxlength="100" tabindex="1" />
